 ...
 *  H5C3 Framework
 *  Plugin
 * Class:  h5c3.plugin.Physics
 *  h5c3.Plugin
 * 
 * Physics class for extended from h5c3.Plugin to provide functionality to the h5c3.
 *
 * 
 * var color = new h5c3.Color([255, 0, 0]); // super red
 * (end)
 */
h5c3.plugin.Physics = h5c3.Plugin.extend('h5c3.plugin.Physics',
{
	srcDir: '../plugins/'
},
{
...
Box2D.Common.b2Settings.b2Assert = function (a)
{
    if (!a)
    {
        throw"box2dweb.2.1a-h5c3.js::Box2D.Common.b2Settings.b2Assert() - Assertion Failed";
    }
};
...
Box2D.Dynamics.b2dbgDraw = function ()
{
...
Box2D.Dynamics.b2dbgDraw.prototype.SetOrigin = function(x, y)
{
    this.m_originX = x;
    this.m_originY = y;
};

Box2D.Dynamics.b2dbgDraw.prototype.Clear = function ()
{
//  this.m_ctx.clearRect(0, 0, this.m_ctx.canvas.width, this.m_ctx.canvas.height)
};
Box2D.Dynamics.b2dbgDraw.prototype._color = function (a, b)
{
    return"rgba(" + ((a & 16711680) >> 16) + "," + ((a & 65280) >> 8) + "," + (a & 255) + "," + b + ")"
};
Box2D.Dynamics.b2dbgDraw.prototype.SetFlags = function (a)
{
    this.m_drawFlags = a
};
Box2D.Dynamics.b2dbgDraw.prototype.GetFlags = function ()
{
    return this.m_drawFlags
};
Box2D.Dynamics.b2dbgDraw.prototype.AppendFlags = function (a)
{
    this.m_drawFlags |= a
};
Box2D.Dynamics.b2dbgDraw.prototype.ClearFlags = function (a)
{
    this.m_drawFlags &= ~a
};
Box2D.Dynamics.b2dbgDraw.prototype.SetSprite = function (a)
{
    this.m_ctx = a
};
Box2D.Dynamics.b2dbgDraw.prototype.GetSprite = function ()
{
    return this.m_ctx
};
Box2D.Dynamics.b2dbgDraw.prototype.SetDrawScale = function (a)
{
    this.m_drawScale = a
};
Box2D.Dynamics.b2dbgDraw.prototype.GetDrawScale = function ()
{
    return this.m_drawScale
};
Box2D.Dynamics.b2dbgDraw.prototype.SetLineThickness = function (a)
{
    this.m_lineThickness = a;
    this.m_ctx.strokeWidth = a
};
Box2D.Dynamics.b2dbgDraw.prototype.GetLineThickness = function ()
{
    return this.m_lineThickness
};
Box2D.Dynamics.b2dbgDraw.prototype.SetAlpha = function (a)
{
    this.m_alpha = a
};
Box2D.Dynamics.b2dbgDraw.prototype.GetAlpha = function ()
{
    return this.m_alpha
};
Box2D.Dynamics.b2dbgDraw.prototype.SetFillAlpha = function (a)
{
    this.m_fillAlpha = a
};
Box2D.Dynamics.b2dbgDraw.prototype.GetFillAlpha = function ()
{
    return this.m_fillAlpha
};
Box2D.Dynamics.b2dbgDraw.prototype.SetXFormScale = function (a)
{
    this.m_xformScale = a
};
Box2D.Dynamics.b2dbgDraw.prototype.GetXFormScale = function ()
{
...
})(Box2D.Dynamics.b2dbgDraw);
Box2D.Dynamics.b2dbgDraw.e_shapeBit = 1;
Box2D.Dynamics.b2dbgDraw.e_jointBit = 2;
Box2D.Dynamics.b2dbgDraw.e_aabbBit = 4;
Box2D.Dynamics.b2dbgDraw.e_pairBit = 8;
Box2D.Dynamics.b2dbgDraw.e_centerOfMassBit = 16;
Box2D.Dynamics.b2dbgDraw.e_controllerBit = 32;
...
Box2D.Dynamics.b2World = function (a, b)
{
    ...
    this.m_dbgDraw = this.m_destructionListener = null;
    this.bodyList = new Box2D.Dynamics.b2BodyList;
    this.contactList = new Box2D.Dynamics.Contacts.b2ContactList;
    this.m_jointList = null;
    this.controllerList = new Box2D.Dynamics.Controllers.b2ControllerList;
...
Box2D.Dynamics.b2World.prototype.SetdbgDraw = function (a)
{
    this.m_dbgDraw = a
};
...
Box2D.Dynamics.b2World.prototype.AddController = function (a)
{
    ...
    {
        throw Error("box2dweb.2.1a-h5c3.js::Box2D.Dynamics.b2World.prototype.AddController() - Controller can only be a member of one world");
    }
    this.controllerList.AddController(a);
    a.m_world = this;
    return a
...
{
    if (null !== this.m_dbgDraw)
    {
        this.m_dbgDraw.Clear();
        var a = this.m_dbgDraw.GetFlags();
        if (a & Box2D.Dynamics.b2dbgDraw.e_shapeBit)
        {
        ...
        if (a & Box2D.Dynamics.b2dbgDraw.e_jointBit)
        {
        ...
        if (a & Box2D.Dynamics.b2dbgDraw.e_controllerBit)
        {
            for (g = this.controllerList.GetFirstNode(); g; g = g.GetNextNode())
            {
                g.controller.Draw(this.m_dbgDraw)
            }
        }
        if (a & Box2D.Dynamics.b2dbgDraw.e_pairBit)
        {
            ...
            {
                b = i.contact.GetFixtureA(), h = i.contact.GetFixtureB(), b = b.GetAABB().GetCenter(), h = h.GetAABB().GetCenter(), this.m_dbgDraw.DrawSegment(b, h, g), Box2D.Common.Math.b2Vec2.Free(b), Box2D.Common.Math.b2Vec2.Free(h)
            }
        }
        if (a & Box2D.Dynamics.b2dbgDraw.e_aabbBit)
        {
            ...
            for (g = this.bodyList.GetFirstNode(Box2D.Dynamics.b2BodyList.TYPES.activeBodies); g; g = g.GetNextNode())
            {
                ...
                {
                    j = i.fixture, h = this.m_contactManager.m_broadPhase.GetFatAABB(j.m_proxy), h = [Box2D.Common.Math.b2Vec2.Get(h.lowerBound.x, h.lowerBound.y), Box2D.Common.Math.b2Vec2.Get(h.upperBound.x, h.lowerBound.y), Box2D.Common.Math.b2Vec2.Get(h.upperBound.x, h.upperBound.y), Box2D.Common.Math.b2Vec2.Get(h.lowerBound.x, h.upperBound.y)], this.m_dbgDraw.DrawPolygon(h, 4, b), Box2D.Common.Math.b2Vec2.Free(h[0]), Box2D.Common.Math.b2Vec2.Free(h[1]), Box2D.Common.Math.b2Vec2.Free(h[2]), Box2D.Common.Math.b2Vec2.Free(h[3])
                }
            }
        }
        if (a & Box2D.Dynamics.b2dbgDraw.e_centerOfMassBit)
        {
            for (g = this.bodyList.GetFirstNode(Box2D.Dynamics.b2BodyList.TYPES.allBodies); g; g = g.GetNextNode())
            {
                h = g.body, Box2D.Dynamics.b2World.s_xf.R = h.m_xf.R, Box2D.Dynamics.b2World.s_xf.position = h.GetWorldCenter(), this.m_dbgDraw.DrawTransform(Box2D.Dynamics.b2World.s_xf)
            }
        }
    }
};
...
Box2D.Dynamics.b2World.prototype.DrawJoint = function (a)
{
    if (a instanceof Box2D.Dynamics.Joints.b2DistanceJoint || a instanceof Box2D.Dynamics.Joints.b2MouseJoint)
    {
        ...
        this.m_dbgDraw.DrawSegment(b, c, Box2D.Dynamics.b2World.s_jointColor);
        Box2D.Common.Math.b2Vec2.Free(b);
        Box2D.Common.Math.b2Vec2.Free(c)
    } else
    {
        if (a instanceof Box2D.Dynamics.Joints.b2PulleyJoint)
        {
            ...
            this.m_dbgDraw.DrawSegment(d, b, Box2D.Dynamics.b2World.s_jointColor);
            this.m_dbgDraw.DrawSegment(a, c, Box2D.Dynamics.b2World.s_jointColor);
            this.m_dbgDraw.DrawSegment(d, a, Box2D.Dynamics.b2World.s_jointColor);
            Box2D.Common.Math.b2Vec2.Free(b);
            Box2D.Common.Math.b2Vec2.Free(c);
            Box2D.Common.Math.b2Vec2.Free(d);
            Box2D.Common.Math.b2Vec2.Free(a)
        ...
        {
            b = a.GetAnchorA(), c = a.GetAnchorB(), a.GetBodyA() != this.m_groundBody && this.m_dbgDraw.DrawSegment(a.GetBodyA().m_xf.position, b, Box2D.Dynamics.b2World.s_jointColor), this.m_dbgDraw.DrawSegment(b, c, Box2D.Dynamics.b2World.s_jointColor), a.GetBodyB() != this.m_groundBody && this.m_dbgDraw.DrawSegment(a.GetBodyB().m_xf.position, c, Box2D.Dynamics.b2World.s_jointColor), Box2D.Common.Math.b2Vec2.Free(b), Box2D.Common.Math.b2Vec2.Free(c)
        }
    }
};
Box2D.Dynamics.b2World.prototype.DrawShape = function (a, b, c)
{
    if (a instanceof Box2D.Collision.Shapes.b2CircleShape)
    {
        ...
        this.m_dbgDraw.DrawSolidCircle(d, a.m_radius, b.R.col1, c);
        Box2D.Common.Math.b2Vec2.Free(d)
    } else
    {
        if (a instanceof Box2D.Collision.Shapes.b2PolygonShape)
        {
            ...
            this.m_dbgDraw.DrawSolidPolygon(f, e, c);
            for (d = 0; d < e; d++)
            {
        ...
        {
            a instanceof Box2D.Collision.Shapes.b2EdgeShape && (d = Box2D.Common.Math.b2Math.MulX(b, a.GetVertex1()), b = Box2D.Common.Math.b2Math.MulX(b, a.GetVertex2()), this.m_dbgDraw.DrawSegment(d, b, c), Box2D.Common.Math.b2Vec2.Free(d), Box2D.Common.Math.b2Vec2.Free(b))
        }
    }
};
...
/**
 * Class:  h5c3.components.Joint
 * 
 * [Extends <a href='h5c3.components.Component'>h5c3.components.Component</a>]<BR>
 * [Used in <a href='h5c3.systems.Physics'>h5c3.systems.Physics</a>]
 * 
 * Creates a joint that holds to physics entities together.
 */
h5c3.components.Joint = h5c3.components.Component.extend('h5c3.components.Joint',
/** Interface: h5c3.components.Joint */
{
	/**
	 * Construct (or acquires) a new component with the options supplied
	 * Parameters:  {h5c3.Entity} options.attachTo Entity to attach the joint to
	 * Parameters:  {h5c3.Dim} options.offset Dim x, y of the pixel offset the joint is relative to the source entity
	 * Parameters:  {h5c3.Dim} options.attachmentOffset Dim x, y of the offset of the joint on the attached entity
	 * Parameters:  Number options.distance How long the joint is
	 * Parameters:  {h5c3.JointType} options.type Type of joint (h5c3.JointType.WELD, h5c3.JointType.REVOLUTE, h5c3.JointType.DISTANCE)
	 * Parameters:  Number options.dampingRatio Ratio for damping motion
	 * Parameters:  Number options.lowerAngleLimit Limit angular movement this angle at the lowest
	 * Parameters:  Number options.upperAngleLimit Limit angular movement this angle at the highest
	 * Parameters:  Boolean options.enableLimit True is anglular limiting is enabled
	 * Parameters:  Number options.maxMotorTorque Highest torque that motor can power to
	 * Parameters:  Number options.motorSpeed Speed of the motor that drives joint turn
	 * Parameters:  Boolean options.enableMotor Whether to engage the motor
	 * Parameters:  Number options.angle Angle of the joint
	 * Returns: {h5c3.components.Joint} A configured joint component
	 */
...
/** Interface: h5c3.components.Joint.prototype */
{
	...
	distance:0,
	/** Type of joint -- see h5c3.JointType */
	type:0,
	...
	init:function (options)
	{
		...
		this.attachmentOffset = h5c3.Point.create(0, 0);
		if ($VLD(options))
			this.config(options);
	},
	...
	{
		this.attachTo = $CHK(options.attachedTo, null);
		if (options.offset)
			this.offset.setXY($CHK(options.offset.x, 0), $CHK(options.offset.y, 0));
		if (options.attachmentOffset)
			this.attachmentOffset.setXY($CHK(options.attachmentOffset.x, 0), $CHK(options.attachmentOffset.y, 0));

		this.distance = $CHK(options.distance, 0);
		this.type = $CHK(options.type, h5c3.JointType.WELD);

		this.dampingRatio = $CHK(options.dampingRatio, 0);
		this.lowerAngleLimit = $CHK(options.lowerAngleLimit, 0);
		this.upperAngleLimit = $CHK(options.upperAngleLimit, 359);
		this.enableLimit = $CHK(options.enableLimit, false);
		this.maxMotorTorque = $CHK(options.maxMotorTorque, 100);
		this.enableMotor = $CHK(options.enableMotor, false);
		this.motorSpeed = $CHK(options.motorSpeed, 0);
		this.angle = $CHK(options.angle, 0);
	...
	getAngle:function ()
	{
		return h5c3.Math.radToDeg(this._joint.GetJointAngle());
	},
	 ...
	 * Returns: Boolean True is they are on
	 */
	 ...
	 * Returns: Boolean True is the motor is enabled
	 */
...
/**
 * Class:  h5c3.systems.Physics
 * 
 * [Extends <a href='h5c3.systems.System'>h5c3.systems.System</a>]
 * 
 * A 2D physics system for entities. See the <a href='h5c3.components.Physics'>physics component</a> and
 * <a href='/develop/guide/physics'>physics guide</a>.
 */
h5c3.systems.Physics = h5c3.systems.EntitySystem.extend('h5c3.systems.Physics',
/** Interface: h5c3.systems.Physics */
{
...
/** Interface: h5c3.systems.Physics.prototype */
{
	/** the physics world */
	world:null,
	/** current gravity (h5c3.Dim) */
	gravity:null,
	 ...
	 * Parameters:  {h5c3.Dim} options.gravity Level of gravity as a 2D vector (gravity.x, gravity.y)
	 * Parameters:  {h5c3.TileMap} options.tileCollisionMap.tileMap A tile map which will be used to construct tile collisions
	 * Parameters:  Number options.tileCollisionMap.collisionCategory Collision category for the tile map
	 * Parameters:  Number options.tileCollisionMap.collisionMask Collision mask for the tile map
	 * Parameters:  Number options.tileCollisionMap.collisionGroup Collision group for the tile map
	 * Parameters:  Boolean options.debug Whether debugging is enabled
	 */
	init:function (options)
	{
		this._super([ 'physics' ]);
		...
		if (options && options.gravity)
			this.gravity = h5c3.Point.create($CHK(options.gravity.x, 0), $CHK(options.gravity.y, 0));
		else
			this.gravity = h5c3.Point.create(0, 0);
		...
		if (options && $VLD(options.tileCollisionMap))
		{
			h5c3.assert($VLD(options.tileCollisionMap.tileMap), 'A tileMap is required for a tileCollisionMap');
			this.addTileCollisionMap(
				options.tileCollisionMap.tileMap,
				$CHK(options.tileCollisionMap.collisionGroup, 0),
				$CHK(options.tileCollisionMap.collisionCategory, 0),
				$CHK(options.tileCollisionMap.collisionMask, 0));
		}
		...
		// setup debug drawing
		var b2dbgDraw = Box2D.Dynamics.b2dbgDraw;
		this.dbgDraw = new b2dbgDraw();
		this.dbgDraw.SetSprite(h5c3.device.game.ctx);
		this.dbgDraw.SetDrawScale(this.Class.SCALE * 100);
		this.dbgDraw.SetFillAlpha(0.3);
		this.dbgDraw.SetLineThickness(1.0);
		this.dbgDraw.SetFlags(b2dbgDraw.e_shapeBit | b2dbgDraw.e_jointBit | b2dbgDraw.e_centerOfMassBit);

		if (options && options._dbgON == true)
		{
			this._dbgON = options._dbgON;
			this.setDebug(options._dbgON);
		}
	},
	 ...
	 * Parameters:  {h5c3.Entity} entity Entity being processed
	 */
	process:function (entity)
	{
		...
		if (!ph._body)
		{
			...
			ph._body.SetAngle(h5c3.Math.degToRad(sp.dir));
			ph._body.SetUserData(entity);
			ph._body._pc_bodyType = h5c3.BodyType.ENTITY;
			...
			// Fixtures
			//
			h5c3.assert(ph.shapes.length, "You must specify at least one shapes for a physics entity");
			...
			for (var i = 0; i < ph.shapes.length; i++)
			{
				...
				h5c3.assert(hw > 0 && hh > 0, "Physics requires a spatial size minimum of 1");
				...
				switch (shape.shape)
				{
					case h5c3.CollisionShape.CIRCLE:
					...
					default: // h5c3.CollisionShape.RECT:
						fixDef.shape = new Box2D.Collision.Shapes.b2PolygonShape;
			...
			if (ph.centerOfMass.x != 0 || ph.centerOfMass.y != 0 || ph.mass != -1)
			{
				...
				md.center = Box2D.Common.Math.b2Vec2.Get(ph.centerOfMass.x * h5c3.systems.Physics.SCALE, ph.centerOfMass.y * h5c3.systems.Physics.SCALE);
			...
			ph._lastSpatialPos = h5c3.Point.create(sp.pos.x, sp.pos.y);
			ph._lastSpatialDim = h5c3.Dim.create(sp.dim.x, sp.dim.y);
						...
						switch (at.type)
						{
							...
							case h5c3.JointType.REVOLUTE:
								...
								if (at.enableLimit)
								{
									...
									jointDef.lowerAngle = h5c3.Math.degToRad(at.lowerAngleLimit);
									jointDef.upperAngle = h5c3.Math.degToRad(at.upperAngleLimit);
								}

								if (at.enableMotor)
								{
									jointDef.enableMotor = at.enableMotor;
									jointDef.motorSpeed = h5c3.Math.degToRad(at.motorSpeed);
		...
		var dir = Math.round(h5c3.Math.radToDeg(ph._body.GetAngle()));
		...
		{
			ph._body.SetAngle(h5c3.Math.degToRad(sp.dir));
			dir = Math.round(h5c3.Math.radToDeg(ph._body.GetAngle()));
		}
	...
	processAll:function ()
	{
		// this.world.Step(h5c3.device.elapsed / 200, 20, 20);
	...
	/**
	 * Sets debugging
	 * Parameters:  Boolean on True to enable debugging
	 */
	setDebug:function (on)
	{
		if (on)
		{
			this.world.SetdbgDraw(this.dbgDraw);
		} else
			this.world.SetdbgDraw(null);
	 ...
	 * Parameters:  {h5c3.Rect} rect Area to query
	 * Returns: Array Array of entities in the area
	 */
	 ...
	 * Parameters:  Number x x-position of the collidable shape
	 * Parameters:  Number y y-position of the collidable shape
	 * Parameters:  Number w width of the collidable shape
	 * Parameters:  Number h height of the collidable
	 * Parameters:  Number collisionGroup Collision group index
	 * Parameters:  Number collisionCategory Collision category
	 * Parameters:  Number collisionMask Collision mask
	 */
	 ...
	 * Parameters:  {h5c3.BodyType} aType Type of the collision body (h5c3.BodyType.TILE or h5c3.BodyType.ENTITY)
	 * Parameters:  {h5c3.BodyType} bType Type of the collision body (h5c3.BodyType.TILE or h5c3.BodyType.ENTITY)
	 * Parameters:  {h5c3.Entity} entityA If an entity, a reference to the entity that was the first part of the collision
	 * Parameters:  {h5c3.Entity} entityB If an entity, a reference to the entity that was the second part of the collision
	 * Parameters:  Number fixtureAType User type provided when fixture was created of the first fixture
	 * Parameters:  Number fixtureBType User type provided when fixture was created of the second fixture
	 * Parameters:  {b2Contact} contact Additional contact information
	 */
	 ...
	 * Parameters:  {h5c3.Entity} entityA If an entity, a reference to the entity that was the first part of the collision
	 * Parameters:  {h5c3.Entity} entityB If an entity, a reference to the entity that was the second part of the collision
	 * Parameters:  Number force The impact force of the collision
	 * Parameters:  Number fixtureAType User type provided when fixture was created of the first fixture
	 * Parameters:  Number fixtureBType User type provided when fixture was created of the second fixture
	 * Parameters:  {b2Contact} contact Additional contact information
	 */
 ...
 * each shape are:
 * - shape: shape type (h5c3.CollisionShape.RECT | h5c3.CollisionShape.CIRCLE | h5c3.CollisionShape.POLY)
 ...
 * - collision same as component collision group, but applies to only this fixture shape.
 ...
 * 
 * e.addComponent(h5c3.components.Physics.create(
 * {
 *   ...
 *
 *     shapes:[
 *         // upper torso/head
 *         { type:0, offset:{y:-20, w:-60}, shape:h5c3.CollisionShape.CIRCLE },
 *         // middle torso
 *         { type:0, offset:{y:-3, w:-60}, shape:h5c3.CollisionShape.CIRCLE },
 *         // leg area
 *         { type:0, offset:{y:12, w:-60}, shape:h5c3.CollisionShape.CIRCLE },
 *         // feet
 *         { type:1, sensorOnly:true, shape:h5c3.CollisionShape.CIRCLE, offset:{y:20, w:-68} }
 *     ],
 *
 *     ...
 * }));
...
h5c3.components.Physics = h5c3.components.Component.extend('h5c3.components.Physics',
/** Interface: h5c3.components.Physics */
{
	/**
	 * Creates (or acquires) a new physics component using the provided options
	 * Parameters:  Number [options.collisionGroup] Collision group to assign (default: 0)
	 * Parameters:  Number [options.collisionCategory] Collision category to assign (default: 0)
	 * Parameters:  Number [options.collisionMask] Collision mask to assign (default: 0)
	 * Parameters:  Boolean [options.sensorOnly] Don't react to collisions, just sense them (default: false)
	 * Parameters:  Boolean [options.centered] Causes box2d body to be aligned to center (rather than top left shifted) (default: true)
	 * Parameters:  Array [options.shapes] An array of shapes representing the fixtures (default: entity's spatial rectangle)
	 * Parameters:  {h5c3.Dim} [options.maxSpeed] Maxium velocity to allow the entity to go (as an x, y vector)
	 * Parameters:  {h5c3.Dim} [options.gravity] Gravity override for the entity only (x, y vector)
	 * Parameters:  Number [options.mass] Amount of relative mass to assign to the entity
	 * Parameters:  Boolean [options.fixedRotation] True if the object is not allow to turn (default: false)
	 * Parameters:  Number [options.thrust] Initial thrust to apply
	 * Parameters:  Number [options.bounce] Amount of bounciness (2=200% reverse velocity on impact)
	 * Parameters:  Boolean [options.faceVel] Use true to have the entity always face the direction it's heading
	 * Parameters:  {h5c3.CollisionShape} [options.shape] Collision shape default (if shapes array not set)
	 * Parameters:  Boolean [options.immovable] Makes the object immovable (by any force)
	 * Parameters:  Number [options.density] How dense the entity is
	 * Parameters:  Number [options.friction] Amount of friction to apply
	 * Parameters:  Number [options.linearDamping] How fast to slow down velocity (less = more slide)
	 * Parameters:  Number [options.angularDamping] How fast to slow down spin (less = better bearings)
	 * Parameters:  Boolean [options.bullet] Special case handling of high-speed objects (enabled CCD)
	 * Parameters:  Number [options.torque] Amount of torque to apply (generate spin)
	 * Parameters:  Number [options.impulse] Amount of impulse force to apply initially
	 * Parameters:  Number [options.turn] Amount of initial spin to apply
	 * Parameters:  {h5c3.Dim} [options.centerOfMass] Where to position the entities centerOfMass (x, y)
	 */
	create:function (options)
	{
		var n = this._super();
...
/** Interface: h5c3.components.Physics.prototype */
{
	/** maximum speed the entity can move at (h5c3.Dim|h5c3.Point) */
	maxSpeed:null,
	...
	immovable:false,
	/** Changes the center of mass from the default center (h5c3.Dim) */
	centerOfMass:null,
	...
	{
		this._super('physics');
		this.centerOfMass = h5c3.Point.create(0, 0);
		this.margin = { x:0, y:0 };
		if ($VLD(options))
			this.config(options);
		this._velReturn = h5c3.Dim.create(0, 0);
		this.gravity = {};
	},
	...
	config:function (options)
	{
		...
		this.collisionGroup = $CHK(options.collisionGroup, 0);
		this.collisionCategory = $CHK(options.collisionCategory, 0);
		this.collisionMask = $CHK(options.collisionMask, 0);
		this.sensorOnly = $CHK(options.sensorOnly, false);
		this.centered = $CHK(options.centered, true);

		// no shape supplied, create a default one
		if (!$VLD(options.shapes) && !Array.isArray(options.shapes))
		{
			options.shapes = [
				{}
			];
			options.shapes[0].shape = h5c3.CollisionShape.RECT;
		}
		...
		for (var i = 0; i < options.shapes.length; i++)
		{
			...
			shape.offset.x = $CHK(shape.offset.x, 0);
			shape.offset.y = $CHK(shape.offset.y, 0);
			shape.offset.w = $CHK(shape.offset.w, 0);
			shape.offset.h = $CHK(shape.offset.h, 0);

			shape.type = $CHK(shape.type, 0);
			shape.shape = $CHK(shape.shape, h5c3.CollisionShape.RECT);
			shape.sensorOnly = $CHK(shape.sensorOnly, this.sensorOnly);
			shape.collisionGroup = $CHK(shape.collisionGroup, this.collisionGroup);
			shape.collisionCategory = $CHK(shape.collisionCategory, this.collisionCategory);
			shape.collisionMask = $CHK(shape.collisionMask, this.collisionMask);
		}
		...
		if (options.maxSpeed)
		{
			this.maxSpeed.x = $CHK(options.maxSpeed.x, 0);
			this.maxSpeed.y = $CHK(options.maxSpeed.y, 0);
		} else
		{
			this.maxSpeed.x = 0;
			this.maxSpeed.y = 0;
		...
		this.mass = $CHK(options.mass, -1);

		this.fixedRotation = $CHK(options.fixedRotation, false);
		this.thrust = $CHK(options.thrust, 0);
		this.bounce = $CHK(options.bounce, 0.5);
		this.faceVel = $CHK(options.faceVel, 0);
		this.shape = $CHK(options.shape, h5c3.CollisionShape.RECT);

		this.immovable = $CHK(options.immovable, false);

		this.density = $CHK(options.density, 1);
		this.friction = $CHK(options.friction, 0.2);
		this.linearDamping = $CHK(options.linearDamping, 0);
		this.angularDamping = $CHK(options.angularDamping, 0);
		this.bullet = $CHK(options.bullet, false);
		this.torque = $CHK(options.torque, 0);
		this.impulse = $CHK(options.impulse, 0);
		this.turn = $CHK(options.turn, 0);
		this.force = $CHK(options.force, 0);

		if ($VLD(options.centerOfMass))
		{
			this.centerOfMass.x = $CHK(options.centerOfMass.x);
			this.centerOfMass.y = $CHK(options.centerOfMass.y);
		}
	},

	/**
	 * Applies angular velocity (turn) to the physics body
	 * Parameters:  Number d Degrees of turn to apply
	 */
	applyTurn:function (d)
	{
		if (this._body)
		{
			this._body.SetAngularVelocity(h5c3.Math.degToRad(d));
	 ...
	 * balloons.
	 * Parameters:  gravityX Gravity value (0 for no gravity)
	 * Parameters:  gravityY Gravity value (0 for no gravity)
	 */
	 ...
	 * Parameters:  Number d Direction to change to
	 */
	setDir:function (d)
	{
		if (this._body)
		{
			this._body.SetAngle(h5c3.Math.degToRad(d));
			this._body.SetAwake(true);
		} else
			this.dir = d;
	},
	...
	getDir:function ()
	{
		if (this._body)
		{
			return h5c3.Math.radToDeg(this._body.GetAngle());
		}
		return 0;
	},
	 ...
	 * Parameters:  Number f Amount of force to apply
	 * Parameters:  Number a Angle to apply the force at
	 */
	 ...
	 * Parameters:  Number f Amount of force to apply
	 * Parameters:  Number a Direction to apply it at
	 */
	 ...
	 * Parameters:  Number a Amount of angular force
	 */
	applyTorque:function (a)
	{
		if (this._body)
		{
			this._body.ApplyTorque(h5c3.Math.degToRad(a));
		} else
			this.torque = a;
	},
	 ...
	 * Parameters:  Number x x-position relative to the origin of the entity
	 * Parameters:  Number y y-position relative to the origin of the entity
	 */
	setCenterOfMass:function (x, y)
	{
		...
		{
			var md = new Box2D.Collision.Shapes.b2MassData();
			md.center = Box2D.Common.Math.b2Vec2.Get(h5c3.systems.Physics.toP(x), h5c3.systems.Physics.toP(y));
	 ...
	 * Returns: Number Current linear velocity (the length of the speed vector)
	 */
	getSpeed:function ()
	{
		if (this._body)
			return this._body.GetLinearVelocity().Length() / h5c3.systems.Physics.SCALE;
		return 0;
	},

	/**
	 * Force change the speed of the entity
	 * Parameters:  Number x x-component of a speed vector
	 * Parameters:  Number y y-component of a speed vector
	 */
	setLinearVelocity:function (x, y)
	{
		if (this._body)
		{
			this._body.SetLinearVelocity(Box2D.Common.Math.b2Vec2.Get(x * h5c3.systems.Physics.SCALE, y * h5c3.systems.Physics.SCALE));
			this._body.SetAwake(true);
		}
	},
	 ...
	 * Returns: {h5c3.Dim} Current velocity as a 2d vector
	 */
	getLinearVelocity:function ()
	{
		if (this._body)
		{
			...
			this._velReturn.setXY(h5c3.systems.Physics.fromP(v.x), h5c3.systems.Physics.fromP(v.y));
		}
	...
	{
		return h5c3.Math.angleFromVector(this._body.GetLinearVelocity().x, this._body.GetLinearVelocity().y);
	},

	/**
	 * Forces an angular velocity (spin) change
	 * Parameters:  Number a Amount of angular force to apply
	 */
	 ...
	 * Parameters:  Number c Category to change to
	 */
	 ...
	 * Parameters:  Number g Group to change to
	 */
	setCollisionfunction (g)
	{
		if (!this._fixtures.length) return;
	 ...
	 * Parameters:  Number m Mask to change to
	 */
	 ...
	 * Parameters:  Boolean s True if this is only a sensor
	 * Parameters:  Number shapeIndex Index of the shape to change
	 */
